...
namespace Halloween2024
{
	public class ModEntry : Mod
	{
		...
		public override void Entry(IModHelper helper)
		{
			...
			if (!helper.ModRegistry.IsLoaded("blueberry.H2024.CP"))
			{
		...
		public static void SetUpEventLocation(GameLocation where)
		{
			...
			// TODO: Fix
			// [22:58:05 ERROR bbhm.cs] required index 1 (ModEntry.State.Value.MapGoal > x) has value '70 69', which can't be parsed as a number
		...
		public static void TryCreateMapSprite(Event e)
		{
		...
		public static void DrawOverlays(SpriteBatch b)
		{
			Farmer who = Game1.player;
			...
			if (ModEntry.State.Value.MinimapTexture is Texture2D minimap && !ModEntry.IsNightmare)
			{
				...
				float baseScale = 2f;
				float hoverScale = 4f;
				Vector2 minimapSize = minimap.Bounds.Size.ToVector2() * (ModEntry.State.Value.IsMinimapHovered ? hoverScale : baseScale);
				float minimapAlpha = Math.Min(0.666f, Vector2.Distance(who.Position, new(Game1.tileSize)) / minimapSize.X);
				ModEntry.State.Value.IsMinimapHovered = minimapAlpha >= 0.5f && new Rectangle(v.ToPoint(), minimapSize.ToPoint()).Contains(Game1.getMousePosition());
				float minimapScale = ModEntry.State.Value.IsMinimapHovered
					? hoverScale
					: baseScale;
				b.Draw(
					texture: minimap,
					...
					color: Color.White * (ModEntry.State.Value.IsMinimapHovered ? 1f : minimapAlpha),
					rotation: 0f,
					origin: Vector2.Zero,
					scale: minimapScale,
				...
				b.Draw(
					texture: Game1.fadeToBlackRect,
					position: v + who.Tile * minimapScale,
					sourceRectangle: null,
					color: Color.Red,
					rotation: 0f,
					origin: Vector2.Zero,
		...
		{
			if (!e.NewLocation.IsTemporary || e.NewLocation.getMapProperty(ModEntry.Strings.GetValueSafe("ID")) is null)
				return;
		...
		private void Display_Rendered(object? sender, StardewModdingAPI.Events.RenderedEventArgs e)
		{
			...
			ModEntry.DrawOverlays(e.SpriteBatch);
		}
		...
		public static class HarmonyPatches
		{
			...
			public static void Event_UpdateBeforeNextCommand_Postfix(Event __instance)
			{
				...
				ModEntry.TryCreateMapSprite(e: __instance);
			}
			...
			public static void Event_ReceiveActionPress_Postfix(Event __instance, int xTile, int yTile)
			{
				...
				if (ModEntry.State.Value.MapGoal.X == xTile && ModEntry.State.Value.MapGoal.Y == yTile)
				{
					...
					__instance.CurrentCommand += ModEntry.State.Value.IsNaughty ? 2 : 1;
				...
				else
				{
					Game1.activeClickableMenu = new ConfirmationDialog(
						message: ModEntry.Strings.GetValueSafe("Q"),
